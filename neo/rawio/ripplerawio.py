"""
Module for reading data from files in the Ripple in raw format.
...
This IO is able to read:
  * nf1, nf2, .., nf6 files that contain signals at different sampling rates

This IO can handle the following Ripple (Blackrock) file specifications:
  * 2.1
  * 2.2
  * 2.3
...
class RippleRawIO(BaseRawIO):
    ...
    Args:
        filename (string):
            ...
            associate with. Any .nfX or .nev, .sif, or .ccf extensions are
            ignored when parsing this parameter.
        nfx_override (string):
            File name of the .nfX files (without extension). If None,
            filename is used.
            Default: None.
        nev_override (string):
            ...
            filename is used.
            Default: None.
        nfx_to_load (int, list, 'max', 'all' (=None)) default None:
    ...
    Examples:
        >>> reader = RippleRawIO(filename='FileSpec2.3001', nfx_to_load=5)
        >>> reader.parse_header()

            Inspect a set of file consisting of files FileSpec2.3001.nf5 and
            FileSpec2.3001.nev
    ...
    extensions = ['nf' + str(_) for _ in range(1, 7)]
    extensions.extend(['nev', ])  # 'sif', 'ccf' not yet supported
    rawmode = 'multi-file'

    def __init__(
        self, filename=None, nfx_override=None, nev_override=None,
        nfx_to_load=None, verbose=False
    ):
        """
        Initialize the RippleRawIO class.
        """
        ...
        self.nfx_to_load = nfx_to_load

        # remove extensions from overrides
        self._filenames = {}
        if nfx_override:
            self._filenames['nfx'] = re.sub(
                os.path.extsep + 'nf[1,2,3,4,5,6]$', '', nfx_override)
        else:
            self._filenames['nfx'] = self.filename
        if nev_override:
        ...
        self._avail_nfx = []
        for ext in self.extensions:
            if ext.startswith('nf'):
                file2check = ''.join(
                    [self._filenames['nfx'], os.path.extsep, ext])
            else:
                file2check = ''.join(
            ...
            if os.path.exists(file2check):
                ...
                if ext.startswith('nf'):
                    self._avail_nfx.append(int(ext[-1]))

        if not self._avail_nfx:
            raise IOError("No Ripple files found in specified path")

        # These dictionaries are used internally to map the file specification
        # revision of the nfx and nev files to one of the reading routines
        # NFX
        self.__nfx_header_reader = {
            '2.1': self.__read_nfx_header_variant_a,
            '2.2': self.__read_nfx_header_variant_b,
            '2.3': self.__read_nfx_header_variant_b}
        self.__nfx_dataheader_reader = {
            '2.1': self.__read_nfx_dataheader_variant_a,
            '2.2': self.__read_nfx_dataheader_variant_b,
            '2.3': self.__read_nfx_dataheader_variant_b}
        self.__nfx_data_reader = {
            '2.1': self.__read_nfx_data_variant_a,
            '2.2': self.__read_nfx_data_variant_b,
            '2.3': self.__read_nfx_data_variant_b}
        self.__nfx_params = {
            '2.1': self.__get_nfx_param_variant_a,
            '2.2': self.__get_nfx_param_variant_b,
            '2.3': self.__get_nfx_param_variant_b}
        # NEV
    ...
    def _parse_header(self):
        ...
        # Step2 NFX file
        # Load file spec and headers of available nfx files
        self.__nfx_spec = {}
        self.__nfx_basic_header = {}
        self.__nfx_ext_header = {}
        self.__nfx_data_header = {}

        for nfx_nb in self._avail_nfx:
            spec = self.__nfx_spec[nfx_nb] = self.__extract_nfx_file_spec(nfx_nb)
            # read nfx headers
            self.__nfx_basic_header[nfx_nb], self.__nfx_ext_header[nfx_nb] = \
                self.__nfx_header_reader[spec](nfx_nb)

            # Read nfx data header(s)
            # for nsxdef get_analogsignal_shape(self, block_index, seg_index):
            self.__nfx_data_header[nfx_nb] = self.__nfx_dataheader_reader[spec](nfx_nb)

        # nfx_to_load can be either int, list, 'max', all' (aka None)
        # here make a list only
        if self.nfx_to_load is None or self.nfx_to_load == 'all':
            self.nfx_to_load = list(self._avail_nfx)
        elif self.nfx_to_load == 'max':
            if len(self._avail_nfx):
                self.nfx_to_load = [max(self._avail_nfx)]
            else:
                self.nfx_to_load = []
        elif isinstance(self.nfx_to_load, int):
            self.nfx_to_load = [self.nfx_to_load]
        elif isinstance(self.nfx_to_load, list):
            pass
        else:
            raise(ValueError('nfx_to_load is wrong'))

        assert all(nfx_nb in self._avail_nfx for nfx_nb in self.nfx_to_load),\
               'nfx_to_load do not match available nfx list'

        # check that all files come from the same specification
        all_spec = [self.__nfx_spec[nfx_nb] for nfx_nb in self.nfx_to_load]
        if self._avail_files['nev']:
        ...
        # This requires nfx to be parsed already
        # Needs to be called when no nfx are available as well in order to warn the user
        if self._avail_files['nev']:
        ...
        if len(self.nfx_to_load) > 0:
            for nfx_nb in self.nfx_to_load:
                ...
                if spec in ['2.2', '2.3']:
                    ext_header = self.__nfx_ext_header[nfx_nb]
                elif spec == '2.1':
                    ext_header = []
                    keys = [
                        'labels', 'units', 'min_analog_val', 'max_analog_val',
                        'min_digital_val', 'max_digital_val',
                    ]
                    params = self.__nfx_params[spec](nfx_nb)
            ...
            # t_start/t_stop for segment are given by nfx limits or nev limits
            self._sigs_t_starts = {nfx_nb: [] for nfx_nb in self.nfx_to_load}
            ...
            for data_bl in range(self._nb_segment):
                ...
                for nfx_nb in self.nfx_to_load:
                    ...
                    if self.__nfx_data_header[nfx_nb] is None:
                        ...
                        t_start = self.__nfx_data_header[nfx_nb][data_bl]['timestamp'] / \
                            self.__nfx_basic_header[nfx_nb]['timestamp_resolution']
                    t_stop = max(t_stop, t_start + length / self.sig_sampling_rates[nfx_nb])
                    self._sigs_t_starts[nfx_nb].append(t_start)
            ...
            for k, (data, ev_ids) in self.nev_data.items():
                for i in np.unique(ev_ids):
                    ...
                    curr_data = data[mask]
                    if curr_data.size > 0:
                        if max(curr_data['timestamp']) >= max_nev_times.get(
                            i, 0
                        ):
                            max_nev_times[i] = max(curr_data['timestamp'])
                        if min(curr_data['timestamp']) <= min_nev_times.get(
                            i, max_nev_times[i]
                        ):
        ...
        for seg_index in range(self._nb_segment):
            ...
            for c in range(signal_streams.size):
                ...
                sig_ann['description'] = f'AnalogSignal from  nfx{nfx_nb}'
                sig_ann['file_origin'] = self._filenames['nfx'] + '.nf' + str(nfx_nb)
                sig_ann['nfx'] = nfx_nb
                # handle signal array annotations from nfx header
                if self.__nfx_spec[nfx_nb] in ['2.2', '2.3'] and nfx_nb in self.__nfx_ext_header:
                    mask = signal_channels['stream_id'] == stream_id
                    channels = signal_channels[mask]
                    nfx_header = self.__nfx_ext_header[nfx_nb]
                    for key in (
                        'physical_connector', 'connector_pin', 'hi_freq_corner',
                        ...
                        for chan_id in channels['id']:
                            chan_id = int(chan_id)
                            idx = list(nfx_header['electrode_id']).index(chan_id)
                            values.append(nfx_header[key][idx])
    ...
    def _get_signal_t_start(self, block_index, seg_index, stream_index):
        stream_id = self.header['signal_streams'][stream_index]['id']
        nfx_nb = int(stream_id)
        return self._sigs_t_starts[nfx_nb][seg_index]
    ...
    def __extract_nfx_file_spec(self, nfx_nb):
        """
        Extract file specification from an .nfx file.
        """
        ...
        if nfx_file_id['file_id'].decode() == 'NEURALSG':
            ...
            raise IOError('Unsupported nfx file type.')
    ...
    def __read_nfx_header_variant_a(self, nfx_nb):
        """
        Extract nfx header information from a 2.1 .nfx file
        """
        filename = '.'.join([self._filenames['nfx'], 'nf%i' % nfx_nb])
    ...
    def __read_nfx_header_variant_b(self, nfx_nb):
        """
        Extract nfx header information from a 2.2 or 2.3 .nfx file
        """
        filename = '.'.join([self._filenames['nfx'], 'nf%i' % nfx_nb])
    ...
    def __read_nfx_dataheader(self, nfx_nb, offset):
        """
        Reads data header following the given offset of an nfx file.
        """
        filename = '.'.join([self._filenames['nfx'], 'nf%i' % nfx_nb])
    ...
    def __read_nfx_dataheader_variant_a(
            self, nfx_nb, filesize=None, offset=None):
        """
        Reads None for the nfx data header of file spec 2.1. Introduced to
        facilitate compatibility with higher file spec.
        """

        return None

    def __read_nfx_dataheader_variant_b(
            self, nfx_nb, filesize=None, offset=None, ):
        """
        Reads the nfx data header for each data block following the offset of
        ...
        if offset is None:
            offset = self.__nfx_basic_header[nfx_nb]['bytes_in_headers']

        while offset < filesize:
            dh = self.__read_nfx_dataheader(nfx_nb, offset)
            data_header[index] = {
                'header': dh['header'],
            ...
            # data size = number of data points * (4bytes * number of channels)
            # use of `int` avoids overflow problem
            data_size = int(dh['nb_data_points']) *\
                int(self.__nfx_basic_header[nfx_nb]['channel_count']) * 4
    ...
    def __read_nfx_data_variant_a(self, nfx_nb):
        """
        Extract nfx data from a 2.1 .nfx file
        """
        filename = '.'.join([self._filenames['nfx'], 'nf%i' % nfx_nb])

        # get shape of data
        shape = (
            self.__nfx_params['2.1'](nfx_nb)['nb_data_points'],
            self.__nfx_basic_header[nfx_nb]['channel_count'])
        offset = self.__nfx_params['2.1'](nfx_nb)['bytes_in_headers']

        # read nfx data
        # store as dict for compatibility with higher file specs
        data = {0: np.memmap(
            filename, dtype='float32', shape=shape, offset=offset, mode='r')}

        return data

    def __read_nfx_data_variant_b(self, nfx_nb):
        """
        Extract nfx data (blocks) from a 2.2 or 2.3 .nfx file. Blocks can arise
        if the recording was paused by the user.
        """
        filename = '.'.join([self._filenames['nfx'], 'nf%i' % nfx_nb])

        data = {}
        for data_bl in self.__nfx_data_header[nfx_nb].keys():
            # get shape and offset of data
            shape = (
                self.__nfx_data_header[nfx_nb][data_bl]['nb_data_points'],
                self.__nfx_basic_header[nfx_nb]['channel_count'])
            offset = \
                self.__nfx_data_header[nfx_nb][data_bl]['offset_to_data_block']
    ...
    def __read_nev_header(self, ext_header_variants):
        """
        Extract nev header information from a of specific .nfx header variant
        """
        filename = '.'.join([self._filenames['nev'], 'nev'])
    ...
    def __match_nfx_and_nev_segment_ids(self, nfx_nb):
        """
        Ensure matching ids of segments detected in nfx and nev file for version 2.3
        """

        # nfx required for matching, if not available, warn the user
        if not self._avail_nfx:
            warnings.warn(
                "No nfx available so it cannot be checked whether "
        ...
        if self.__nev_spec == '2.3':
            nfx_offset = self.__nfx_data_header[nfx_nb][0]['timestamp']
            # Multiples of 1/30.000s that pass between two nfx samples
            nfx_period = self.__nfx_basic_header[nfx_nb]['period']
            # nfx segments needed as dict and list
            nonempty_nfx_segments = {}
            list_nonempty_nfx_segments = []
            ...
            for k, v in sorted(self.__nfx_data_header[nfx_nb].items()):
                if v['nb_data_points'] > 1:
                    nonempty_nfx_segments[k] = v
                    ...
                    if len(data[mask_after_seg]) > 0:
                        ...
                        # in available files
                            ...
                            if len(data[ev_ids == i]) != len(data[mask_after_seg]):
                                raise ValueError("Some segments in nfx cannot be detected in nev")
    ...
    def __get_min_time(self):
        ...
        for nfx_i in self._avail_nfx:
            tp.extend(self.__nfx_rec_times[self.__nfx_spec[nfx_i]](nfx_i)[0])
    ...
    def __get_max_time(self):
        ...
        for nfx_i in self._avail_nfx:
            tp.extend(self.__nfx_rec_times[self.__nfx_spec[nfx_i]](nfx_i)[1])
    ...
    def __get_nfx_param_variant_a(self, nfx_nb):
        """
        Returns parameter (param_name) for a given nfx (nfx_nb) for file spec
        2.1.
        """
        # Here, min/max_analog_val and min/max_digital_val are not available in
        # the nfx, so that we must estimate these parameters from the
        ...
        labels = []
        dig_factor = []
        for elid in self.__nfx_ext_header[nfx_nb]['electrode_id']:
            if self._avail_files['nev']:
        ...
        bytes_in_headers = self.__nfx_basic_header[nfx_nb].dtype.itemsize + \
            self.__nfx_ext_header[nfx_nb].dtype.itemsize * \
            self.__nfx_basic_header[nfx_nb]['channel_count']
        ...
        nfx_parameters = {
            'nb_data_points': int(
                ...
                / (2 * self.__nfx_basic_header[nfx_nb]['channel_count']) - 1),
            'labels': labels,
            'units': np.array([units] * self.__nfx_basic_header[nfx_nb]['channel_count']),
            'min_analog_val': -1 * np.array(dig_factor),
            'max_analog_val': np.array(dig_factor),
            'min_digital_val': np.array(
                [-1000] * self.__nfx_basic_header[nfx_nb]['channel_count']),
            'max_digital_val': np.array([1000] * self.__nfx_basic_header[nfx_nb]['channel_count']),
            'timestamp_resolution': 30000,
            'bytes_in_headers': bytes_in_headers,
            'sampling_rate': 30000 / self.__nfx_basic_header[nfx_nb]['period'] * pq.Hz,
            'time_unit': pq.CompoundUnit("1.0/{}*s".format(
                30000 / self.__nfx_basic_header[nfx_nb]['period']))}

        # Returns complete dictionary because then it does not need to be called so often
        return nfx_parameters

    def __get_nfx_param_variant_b(self, param_name, nfx_nb):
        """
        Returns parameter (param_name) for a given nfx (nfx_nb) for file spec
        2.2 and 2.3.
        """
        nfx_parameters = {
            'labels':
                self.__nfx_ext_header[nfx_nb]['electrode_label'],
            'units':
                self.__nfx_ext_header[nfx_nb]['units'],
            'min_analog_val':
            ...
            'timestamp_resolution':
                self.__nfx_basic_header[nfx_nb]['timestamp_resolution'],
            'bytes_in_headers':
                self.__nfx_basic_header[nfx_nb]['bytes_in_headers'],
            'sampling_rate':
                self.__nfx_basic_header[nfx_nb]['timestamp_resolution']
                / self.__nfx_basic_header[nfx_nb]['period'] * pq.Hz,
            'time_unit': pq.CompoundUnit("1.0/{}*s".format(
                self.__nfx_basic_header[nfx_nb]['timestamp_resolution']
                / self.__nfx_basic_header[nfx_nb]['period']))}

        return nfx_parameters[param_name]
